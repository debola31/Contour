  ...
  persist(
    (set, get) => ({
      ...
      updatePersonnel: (id: string, updates: Partial<Personnel>) => {
        set((state) => ({
          personnel: state.personnel.map((p) => (p.id === id ? { ...p, ...updates } as Personnel : p)),
        }));
      },
      ...
      updateCustomer: (id: string, updates: Partial<Customer>) => {
        set((state) => ({
          customers: state.customers.map((c) => (c.id === id ? { ...c, ...updates } as Customer : c)),
        }));
      ...
      updateMaterial: (id: string, updates: Partial<Material>) => {
        set((state) => ({
          materials: state.materials.map((m) => (m.id === id ? { ...m, ...updates } as Material : m)),
        }));
      ...
      updateStation: (id: string, updates: Partial<Station>) => {
        set((state) => ({
          stations: state.stations.map((s) => (s.id === id ? { ...s, ...updates } as Station : s)),
        }));
      ...
      updateTemplate: (id: string, updates: Partial<WorkOrderTemplate>) => {
        set((state) => ({
          templates: state.templates.map((t) => (t.id === id ? { ...t, ...updates } as WorkOrderTemplate : t)),
        }));
      ...
      updateWorkOrder: (id: string, updates: Partial<WorkOrder>) => {
        set((state) => ({
          workOrders: state.workOrders.map((wo) => (wo.id === id ? { ...wo, ...updates } as WorkOrder : wo)),
        }));
      },
      ...
      updateInvoice: (id: string, updates: Partial<Invoice>) => {
        set((state) => ({
          invoices: state.invoices.map((inv) => (inv.id === id ? { ...inv, ...updates } as Invoice : inv)),
        }));
      },
      ...
      updateShipment: (id: string, updates: Partial<Shipment>) => {
        set((state) => ({
          shipments: state.shipments.map((s) => (s.id === id ? { ...s, ...updates } as Shipment : s)),
        }));
