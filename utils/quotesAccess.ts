...
export async function convertQuoteToJob(
  ...
  const { data: job, error: jobError } = await supabase
    ...
    .insert({
      ...
      routing_id: jobData.routing_id || null,
  ...
  // 5. Copy operations from routing if routing is specified
  if (jobData.routing_id) {
    try {
      await supabase.rpc('create_job_operations_from_routing', {
        p_job_id: job.id,
        p_routing_id: jobData.routing_id,
      });
    } catch (routingError) {
      console.error('Failed to copy operations from routing:', routingError);
      // Don't fail the entire conversion if operation copy fails
    }
  }

  // 6. Copy attachment if exists
